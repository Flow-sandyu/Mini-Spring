...
public class AnnotationConfigApplicationContext {
    ...
    public Object createBeanAsEarlySingleton(BeanDefinition def) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (value != null) {
                ...
                logger.atDebug().log("the @Value param in Constructor injection: {} type = {} \n {} = {}", def.getBeanClass().getName(), type, value.value(), args[i]);
    ...
    void tryInjectProperties(BeanDefinition def, Class<?> clazz, Object bean, AccessibleObject acc) throws ReflectiveOperationException {
        ...
        if (value != null) {
            ...
            if (field != null) {
                logger.atDebug().log("AAA Field injection: {}.{} = {}", def.getBeanClass().getName(), accessibleName, propValue);
                field.set(bean, propValue);
            }
            if (method != null) {
                logger.atDebug().log("BBB Method injection: {}.{} ({})", def.getBeanClass().getName(), accessibleName, propValue);

                // 验证bean是什么的日志
                // logger.atDebug().log("BBB Method injection: {}.{} ({}) - {}", def.getBeanClass().getName(), accessibleName, propValue, bean);

                // bean 是前面创建好的对象，但没注入属性
