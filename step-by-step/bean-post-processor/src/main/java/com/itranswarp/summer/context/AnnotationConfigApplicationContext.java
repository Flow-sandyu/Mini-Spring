...
public class AnnotationConfigApplicationContext {
    ...
    public AnnotationConfigApplicationContext(Class<?> configClass, PropertyResolver propertyResolver) {
        ...
        List<BeanPostProcessor> processors = this.beans.values().stream()
                // 过滤出BeanPostProcessor: 比如 FirstProxyBeanPostProcessor 这种
                .filter(this::isBeanPostProcessorDefinition)
                // 排序:
                // 默认就是 .sorted(BeanDefinition::compareTo)，
                // 排完是 order = 100 -》 order = 200
                .sorted()
    ...
    public Object createBeanAsEarlySingleton(BeanDefinition def) {
        ...
        // 先 FirstProxyBeanPostProcessor（order100） 再 SecondProxyBeanPostProcessor（order200）
        for (BeanPostProcessor processor : beanPostProcessors) {
            ...
            if (def.getInstance() != processed) {
                ...
                // 经过 FirstProxyBeanPostProcessor 后，def.getInstance() = FirstProxyBean
    ...
    protected Set<String> scanForClassNames(Class<?> configClass) {
        ...
        logger.atInfo().log("扫描的日志 component scan in packages: {}", Arrays.toString(scanPackages));
    ...
    // 比如说要取 OriginBean 的原始 bean
    private Object getProxiedInstance(BeanDefinition def) {
        // 猜测这里是 SecondProxyBean
        Object beanInstance = def.getInstance();
        // 如果Proxy改变了原始Bean，又希望注入到原始Bean，则由BeanPostProcessor指定原始Bean:
        List<BeanPostProcessor> reversedBeanPostProcessors = new ArrayList<>(this.beanPostProcessors);
        // 倒序之后 【 SecondProxyBeanPostProcessor（order 200） ，FirstProxyBeanPostProcessor（order100）】
        Collections.reverse(reversedBeanPostProcessors);
        for (BeanPostProcessor beanPostProcessor : reversedBeanPostProcessors) {
            // 先用 SecondProxyBeanPostProcessor 的 postProcessOnSetProperty（ SecondProxyBean@xxx，originBean ）
            Object restoredInstance = beanPostProcessor.postProcessOnSetProperty(beanInstance, def.getName());
            if (restoredInstance != beanInstance) {
                logger.atDebug().log("看beanInstance {} ||| BeanPostProcessor {} specified injection from {} to {}.", def.getName(), beanPostProcessor.getClass().getSimpleName(),
